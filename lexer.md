...
digraph CompilationPipeline {
    ...
    lexer [label="Лексический анализатор", color=gold, style=filled];
    ...
    lexer -> parser [label="Токены", color=red, penwidth=2.0];
    parser -> ast;
    ast -> semantic;
    ...
    subgraph cluster_lexer {
        label="Детализация лексического анализа";
        style=dashed;
        color=gray;
        
        input [label="Входной текст", shape=ellipse];
        tokenizer [label="Токенизатор"];
        classifier [label="Классификатор токенов"];
        output [label="Список токенов", shape=ellipse];
...
digraph LexerStateMachine {
    ...
    initial [label="Начальное", shape=doublecircle];
    identifier [label="Идентификатор"];
    number [label="Число"];
    string [label="Строка"];
    operator [label="Оператор"];
    comment [label="Комментарий"];
    error [label="Ошибка", color=lightcoral];
    final [label="Финальное", shape=doublecircle];
    
    initial -> identifier [label="буква/подчеркивание"];
    initial -> number [label="цифра"];
    initial -> string [label="кавычка"];
    initial -> operator [label="+ - * / = < >"];
    initial -> comment [label="//"];
    initial -> error [label="недопустимый символ"];
    
    identifier -> identifier [label="буква/цифра/подчеркивание"];
    identifier -> final [label="разделитель"];
    
    number -> number [label="цифра"];
    number -> number [label="x (для hex)"];
    number -> final [label="разделитель"];
    
    string -> string [label="любой символ кроме кавычки"];
    string -> final [label="кавычка"];
    
    operator -> operator [label="= (для ==)"];
    operator -> final [label="любой символ"];
    
    comment -> comment [label="любой символ кроме \\n"];
    comment -> final [label="\\n"];
}
```
