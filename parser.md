...
digraph CompilationPipeline {
    ...
    parser [label="Синтаксический\nанализатор (Парсер)", color=gold, style=filled];
    ast [label="Абстрактное\nСинтаксическое Дерево"];
    semantic [label="Семантический\nанализатор"];
    optimization [label="Оптимизация"];
    codegen [label="Генерация кода"];
    
    source -> lexer [label="Символы"];
    lexer -> parser [label="Токены"];
    parser -> ast [label="Создание AST", color=red, penwidth=2.0];
    ast -> semantic;
    semantic -> optimization;
    optimization -> codegen;
    
    subgraph cluster_parser {
        label="Внутренние компоненты парсера";
        style=dashed;
        color=gray;
        
        input [label="Входные токены", shape=ellipse];
        grammar [label="Правила грамматики"];
        parsing_engine [label="Механизм разбора"];
        tree_builder [label="Построитель AST"];
        output [label="Дерево AST", shape=ellipse];
...
digraph PEGParserModel {
    ...
    start [label="Начало парсинга", shape=ellipse];
    token_stream [label="Поток токенов"];
    rule_selection [label="Выбор правила грамматики"];
    matching [label="Сопоставление с паттерном"];
    choice [label="Упорядоченный выбор", shape=diamond];
    success [label="Успешное сопоставление", color=lightgreen];
    failure [label="Неудачное сопоставление", color=lightcoral];
    backtracking [label="Возврат (backtracking)"];
    ast_building [label="Построение AST"];
    end [label="Завершение парсинга", shape=ellipse];
    
    start -> token_stream -> rule_selection -> matching;
    matching -> choice;
    choice -> success [label="Нашли"];
    choice -> failure [label="Не нашли"];
    failure -> backtracking -> rule_selection [label="Пробуем другое правило"];
